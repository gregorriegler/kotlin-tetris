...
class AnchorFinder(val area: Area) {
    ...
    private fun hasAnchorToTheRight(field: Field): Boolean =
        field.falls() && (
                isAnchor(field.position)
                        || hasAnchorToTheRight(area.below(field.position))
                        || hasAnchorToTheRight(area.rightOf(field.position))
                )

    private fun hasAnchorToTheLeft(field: Field): Boolean =
        field.falls() && (
                isAnchor(field.position)
                        || hasAnchorToTheLeft(area.below(field.position))
                        || hasAnchorToTheLeft(area.leftOf(field.position))
                )
