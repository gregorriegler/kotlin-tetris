...
class `A Frame` {
    ...
    fun `returns the top center field`() {
        ...
        assertEquals(Field.filled(Position(1, 0)), frame.topCenterFilled())
    }
    ...
    fun `moves an area to the left`() {
        val frame = TetrisFrame(3, 3)
        assertEquals(
            Area(Field.empty(Position(-1, 0)), Field.filled(Position(0, 0)), Field.filled(Position(1, 0))),
            frame.left(
                Area("""
                -##
                """),
    ...
    fun `moves inside debris when moving left`() {
        ...
        val y = 0
        val expected = Area(
            Field.empty(Position(x, y)),
            Field.empty(Position(0, 0)),
            Field.empty(Position(1, 0)),
            Field.filled(Position(2, 0)),
            Field.empty(Position(-1, 1)),
            Field.empty(Position(0, 1)),
            Field.filled(Position(1, 1)),
            Field.filled(Position(2, 1)),
            Field.empty(Position(-1, 2)),
            Field.empty(Position(0, 2)),
            Field.empty(Position(1, 2)),
    ...
    fun `knows an area is not at the bottom`() {
        assertFalse(TetrisFrame(3, 3).isAtBottom(Area(Field.filled(Position(1, 1)))))
    ...
    fun `knows an area is at the bottom`() {
        assertTrue(TetrisFrame(2, 2).isAtBottom(Area(Field.filled(Position(1, 1)))))
